...
- **Relational Integrity:** Implemented `ON DELETE CASCADE` foreign key constraints connecting messages to their parent chats.
- **Database Indexing:** Added indexes on `messages.chat_id` and `messages.created_at` to guarantee optimal retrieval performance.
- **Schema Refinements:** Enabled `pgcrypto` extension for robust UUID generation, restricted message roles via `CHECK` constraints, and documented automated behaviors.
...
- **Escape Key Support:** Pressing the `Escape` key while focused in the search bar instantly clears the query.
- **Scroll Preservation:** The sidebar will no longer violently jump to the top while you are typing a search query.
- **Focus Management:** Clicking anywhere inside the search container automatically focuses the input field.


## [v5.2.0] - 2026-07-18
### Added
- **Real-Time Sidebar Filtering:** The sidebar now dynamically updates as you type in the search bar, hiding non-matching chats instantly.
- **Empty Search States:** If a search yields no results, the sidebar will gracefully display a "No chats found." message instead of rendering a blank column.
